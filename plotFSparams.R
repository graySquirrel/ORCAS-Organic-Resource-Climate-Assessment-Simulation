...
plotplustext <- function(labels,path,x,y,xl,yl) {
    ...
    text(x=x, y=jitter(y-.02,factor=10),labels = labels,cex=0.75)
}
makeplots <- function(zsize,tit){
    par(mfrow=c(2,2),oma=c(0,0,2,0))
    plotplustext(labels=o$f1$type,path=zsize,x=o$f1$InitialC,y=o$f1$fdeg,xl="InitialC",yl="fdeg")
    plotplustext(labels=o$f1$type,path=zsize,x=o$f1$fdeg,y=o$f1$Lo,xl="fdeg",yl="Lo")
