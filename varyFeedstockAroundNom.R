...
par(mfrow=c(4,2))
modelQ1<-lm(out[,1] ~ inputs$TS)
plot(inputs$TS,out[,1],xlab="TS",ylab="Net Emissions")
abline(modelQ1,lwd=2)
boxplot(out[,1] ~ signif(inputs$TS,2),xlab="TS",ylab="Net Emissions")

modelQ1<-lm(out[,1] ~ inputs$VS)
plot(inputs$VS,out[,1],xlab="VS",ylab="Net Emissions")
abline(modelQ1,lwd=2)
boxplot(out[,1] ~ signif(inputs$VS,2),xlab="VS",ylab="Net Emissions")

modelQ1<-lm(out[,1] ~ inputs$Bo)
plot(inputs$Bo,out[,1],xlab="Bo",ylab="Net Emissions")
abline(modelQ1,lwd=2)
boxplot(out[,1] ~ signif(inputs$Bo,2),xlab="Bo",ylab="Net Emissions")

modelQ1<-lm(out[,1] ~ inputs$TKN)
plot(inputs$TKN,out[,1],xlab="TKN",ylab="Net Emissions")
abline(modelQ1,lwd=2)
